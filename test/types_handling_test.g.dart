...
class A extends JsInterface implements _A {
  ...
  B execute2(B f(B b, [int i])) => __codec1
      .decode(asJsObject(this).callMethod('execute', [__codec7.encode(f)]));
...
/// codec for (B, [int]) → B
final __codec7 = new FunctionCodec /*<(B, [int]) → B>*/ ((f) => (p_b, [p_i]) {
  p_b = __codec1.decode(p_b);
  final result = f(p_b, p_i);
  return __codec1.encode(result);
}, (JsFunction f) => (p_b, [p_i]) {
  p_b = __codec1.encode(p_b);
  final result = f.apply([p_b, p_i]);
  return __codec1.decode(result);
});
